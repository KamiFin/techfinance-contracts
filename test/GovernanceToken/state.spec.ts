...
describe('GovernanceToken::State', () => {
  ...
  let token: Contract
  beforeEach(async () => {
    token = await deployContract(alice, GovernanceToken, ["TechFi", "TFI", TOTAL_CAP, MANUAL_MINT_LIMIT, LOCK_FROM_BLOCK, LOCK_TO_BLOCK])
  ...
  it('should have correct values for: name, symbol, decimals, totalSupply, balanceOf', async () => {
    ...
    expect(name).to.eq('TechFi')
    expect(await token.symbol()).to.eq('TFI')
